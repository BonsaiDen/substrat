...
util.inherit(Substrat, EventEmitter, {
    ...
    _initBuild: function(initial, changedFiles) {
        ...
        if (changedFiles.length) {
            ...
            var that = this;
            this._buildLocalChanges.values().forEach(function(m) {
                that._buildChanges[m.action].add(m.filepath);
            });
    ...
    _getChanges: function() {
        ...
        this._buildChanges.add.values().forEach(function(file) {
            that._mapper.mappings([file]).forEach(function(mapped) {
                changes.added.push(mapped);
            });
        });

        this._buildChanges.change.values().forEach(function(file) {
            that._mapper.mappings([file]).forEach(function(mapped) {
                changes.changed.push(mapped);
            });
        });

        this._buildChanges.unlink.values().forEach(function(file) {
            that._mapper.mappings([file]).forEach(function(mapped) {
