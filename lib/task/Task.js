...
util.inherit(Task, EventEmitter, {
    ...
    run: function(substrat, mapper, options, files, silent) {
            ...
            util.async(files, function(file, next) {
                ...
                read(file, function(err, data) {
                    ...
                    if (err) {
                        ...
                        e.file = file;
                        e.data = data;
                        e.name = handler.map(e, file);
                        e.source = path.join(options.src, file);

                        mapper.update(e.file, e.name);
                        this.invoke(e, next);
                    }
        ...
        // Run the task once for all files at once
            ...
            e.all = [];
            e.name = handler.map(e, this._config.file);
            ...
            util.async(files, function(file, next) {
                ...
                read(file, function(err, data) {
                    ...
                    if (err) {
                        ...
                        e.all.push({
                            file: file,
                            source: path.join(options.src, file),
                            data: data
                        });
                        mapper.update(file, e.name);
                        next();
                    }
    ...
    invoke: function(e, done) {
        ...
        this._handler.run(e, function(err, names, data) {

            if (err) {
                done(err);

            } else if (names) {
                writes(e.options.dest, names, data, done);
